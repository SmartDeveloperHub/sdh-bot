...
'use strict';
    ...
    controller.hears(['users', 'members'],'direct_message,direct_mention,mention',function(bot, message) {
        // "sdh-bot is typing..." msg
        bot.reply(message,{
            "id": 1,
            "type": "typing",
            "channel": "C024BE91L"
        });
        var usersMarkDwn = getUserMarkdown(knownUsers);
        bot.reply(message, {
            "attachments": usersMarkDwn
        });
    });
    ...
    var getUserMarkdown = function getUserMarkdown(userList, imageurl, metricLabel, rangeLabel) {
        ...
        for(var i = 0; i < userList.length; i++){
            ...
            var mdUser = {
                ...
                //"image_url": "http://s30.postimg.org/4h48z9rht/cblanco_activitylastyear3.png"
            };
            if (imageurl) {
                ...
                mdUser["author_icon"] = userList[i].sdh_avatar;
            }
    ...
    controller.hears(["give me (.*)"],'direct_message,direct_mention,mention',function(bot, message) {
        ...
        if (tags.indexOf("activity") > -1) {
            ...
            //var imageurl = "http://s21.postimg.org/nrriyp0fb/cblanco_activitylastyear.png";
            //var imageurl = "http://s21.postimg.org/c210xe7hj/cblanco_activitylastyear2.png";
            var imageurl = "http://s30.postimg.org/4h48z9rht/cblanco_activitylastyear3.png";
            var attach = getUserMarkdown(userList, imageurl, "Activity", "Last Year; (18 Mar 2015  -  31 Dic 2015)");
        } else {
            var attach = getUserMarkdown(userList);
        }
        if (userList.length == 0) {
            bot.reply(message, "Sorry, I don't recognize this name. I only know the SDH members with the same nick in SDH and Slack platforms");
        } else {
            bot.reply(message, {
                "attachments": attach
            });
